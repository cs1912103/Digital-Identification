...
export default function UnionLogin(){
  ...
  const handleSubmit = async (e) => {
    ...
    let _wallet = await contract.methods.educationDeptAddress().call();
    if(_wallet !== accounts[0]){
        alert("You are not belong to Education Department!")
    }
    else if(_wallet !== pass){
      alert("You are passing wrong password! ")
    }
    else{
      window.localStorage.setItem("education","true");
