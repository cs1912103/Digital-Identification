...
export default function UnionLogin(){
  ...
  const handleSubmit = async (e) => {
    ...
    let _wallet = await contract.methods.unionDeptAddress().call();
    if(_wallet !== accounts[0]){
        alert("You are not belong to Union Council Department!")
    }
    else if(_wallet !== pass){
      alert("You are passing wrong password! ")
    }
    else{
      window.localStorage.setItem("union","true");
      window.location.href = "/show-union";
    }
